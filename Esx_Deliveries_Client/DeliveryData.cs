...
namespace Esx_Deliveries_Client
{
    class DeliveryData
    {
        ...
        {
            ["DELIVERY_NOT_AVAILABLE"] = " Tento zpusob zatim ~r~neni dostupny~w~. Pouzijte prosim ~g~MOTORKU~w~ nebo ~g~DODAVKU~w~.",
            ["START_DELIVERY"] = "Zmackni ~INPUT_CONTEXT~ pro spusteni rozvozu zasilek.Bude potreba pronajmout vuz za ~g~$",
            ["SAFE_DEPOSIT_RECEIVED"] = "Zaloha za vozidlo vam byla strzena z uctu.",
            ["SAFE_DEPOSIT_RETURNED"] = "Zaloha za vozidlo vam byla prictena na ucet.",
            ["SAFE_DEPOSIT_WITHHELD"] = "Rozvoz ~r~SELHAL~w~. Zaloha za vozidlo byla ~r~ZADRZENA~w~ bankou.",
            ["DELIVERY_POINT_REWARD"] = "Rozvoz dokončen. Dostal jsi ~g~$",
            ["GET_BACK_IN_VEHICLE"] = "Vrat se do vozidla!",
            ["REMOVE_GOODS"] = "Zmackni ~INPUT_CONTEXT~ pro zvednuti ~y~ZASILKY~w~.",
            ["DELIVER_INSIDE_SHOP"] = "Jdi do obchodu a doruc ~y~ZASILKU~w~.",
            ["GET_BACK_TO_DELIVERYHUB"] = "Vrat se na depo aby jsi mohl vratit vozidlo.",
            ["DELIVERY_VEHICLE_RETURNED"] = "Vratil jsi zapujcene vozidlo.",
            ["END_DELIVERY"] = "Zmackni ~INPUT_CONTEXT~ pro ukonceni dorucovani. Sed ve vozidle, jinak nedostanes zpet zalohu.",
        };
